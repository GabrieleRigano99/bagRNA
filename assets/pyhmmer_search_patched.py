...
from dbcan.constants import HMMER_COLUMN_NAMES
            ...
            for hits in pyhmmer.hmmsearch(hmm_files, targets, cpus=self.hmmer_cpu, domE=self.e_value_threshold):
                for hit in hits:
                    for domain in hit.domains.included:
                        ...
                        _hmm_name_raw = domain.alignment.hmm_name
                        hmm_name = _hmm_name_raw if isinstance(_hmm_name_raw, str) else _hmm_name_raw.decode('utf-8')
                        if "GT2_" in hmm_name:
                            hmm_name = "GT2.hmm"
                        hmm_length = domain.alignment.hmm_length
                        _tgt_name_raw = domain.alignment.target_name
                        target_name = _tgt_name_raw if isinstance(_tgt_name_raw, str) else _tgt_name_raw.decode('utf-8')
...
class PyHMMERDBCANProcessor(PyHMMERProcessor):
    ...
    def _derive_hmm_file(self):
        ...
        return os.path.join(self.config.db_dir, "dbCAN.hmm")
    ...
    def _derive_output_file(self):
        ...
        return os.path.join(self.config.output_dir, "dbCAN_hmm_results.tsv")
...
class PyHMMERDBCANSUBProcessor(PyHMMERProcessor):
    ...
    def _derive_hmm_file(self):
        ...
        return os.path.join(self.config.db_dir, "dbCAN-sub.hmm")
    ...
    def _derive_output_file(self):
        """Get output file path"""
        return os.path.join(self.config.output_dir, "dbCANsub_hmm_results.tsv")
    ...
    def _derive_mapping_file(self):
        ...
        return os.path.join(self.config.db_dir, "fam-substrate-mapping.tsv")
...
class PyHMMERTFProcessor(PyHMMERProcessor):
    ...
    def _derive_hmm_file(self):
        ...
        return os.path.join(self.config.db_dir, "TF.hmm")
    
    def _derive_input_faa(self):
        """Get input protein sequence file path"""
        return os.path.join(self.config.output_dir, "non_CAZyme.faa")
    
    def _derive_output_file(self):
        """Get output file path"""
        return os.path.join(self.config.output_dir, "TF_hmm_results.tsv")
...
class PyHMMERSTPProcessor(PyHMMERProcessor):
    ...
    def _derive_hmm_file(self):
        ...
        return os.path.join(self.config.db_dir, "STP.hmm")
    ...
    def _derive_output_file(self):
        """Get output file path"""
        return os.path.join(self.config.output_dir, "STP_hmm_results.tsv")
